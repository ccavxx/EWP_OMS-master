            ...
            try:
                ...
                if sapi.SaltCmd(client='local', tgt=tgt, fun='file.directory_exists', arg=path)['return'][0][tgt]:
                    ...
                    if path_str[-1] == '..':  # 返回上层
                        ...
                        dir = path
                    svn_info = \
                    sapi.SaltCmd(client='local', tgt=tgt, fun='svn.info', arg=dir, arg1='fmt=dict')['return'][0][tgt][0]
                    if isinstance(svn_info, dict):
                        context['svn'] = {'URL': svn_info['URL'], 'Revision': svn_info['Revision'],
                                          'LastChangedRev': svn_info['Last Changed Rev'],
                                          'LastChangeDate': svn_info["Last Changed Date"][0:20]}
                # 文件存在时，返回文件内容，加上文件格式、大小限制
                    ...
                    if os.path.splitext(path)[1] in FILE_FORMAT:
                        ...
                        if stats['size'] <= 1024000:
                            content = \
                            sapi.SaltCmd(client='local', tgt=tgt, fun='cmd.run', arg='cat ' + path)['return'][0][tgt]
                    ...
                    if len(path_str) > 2:
                        ...
                        dir = '/'
                    svn_info = sapi.SaltCmd(client='local', tgt=tgt, fun='svn.info', arg=dir, arg1='fmt=dict',
                                            arg2='targets=%s' % path_str[-1])['return'][0][tgt][0]
                    if isinstance(svn_info, dict):
                        context['svn'] = {'URL': svn_info['URL'], 'Revision': svn_info['Revision'],
                                          'LastChangedRev': svn_info['Last Changed Rev'],
                                          'LastChangeDate': svn_info["Last Changed Date"][0:20]}
...
def file_remote_rename(request):
    if request.is_ajax() and request.method == 'GET':
        ...
        try:
            ...
            r = sapi.SaltCmd(tgt, 'file.rename', client='local', arg1=path.encode("utf-8"), arg2=dst.encode("utf-8"))[
                'return'][0][tgt]
            if r == True:
    ...
    if request.is_ajax() and request.method == 'GET':
        ...
        try:
            ...
            if sapi.SaltCmd(tgt, 'file.file_exists', client='local', arg=path.encode("utf-8"))['return'][0][tgt]:
                r = sapi.SaltCmd(tgt, 'file.write', client='local', arg1=path.encode("utf-8"),
                                 arg2=content.encode("utf-8"))['return'][0][tgt]
                if re.search('1', r):
